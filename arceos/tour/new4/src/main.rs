...
fn main() {
    ...
    // 从第一个实验开始，我们有意忽略了一个事实：内核程序并不是从 main 开始执行的。
    // 有许多迹象其实暗示了这一点：
    // a. 在第二个实验的 EXERCISE 1 的第一题中，即使不设置 stvec，非法访问依然触发了异常报错，只是位置不同。
    // b. 在第二个实验的虚拟地址部分的输出中，每个 buffer 的虚拟地址和物理地址本就不同。
    ...
    // c. 在每个实验的开头第二行都有 #![cfg_attr(feature = "axstd", no_main)]。
    // 根据第三个实验的 Rust 属性解析，这表示只要 feature 包含 axstd，那么就为本文件添加 no_main 属性。此处本不应该有一个 main 函数存在。 
    ...
    // 本节实验我们主要讲上述的第二个部分，即从内核启动到 main 的过程中，发生了什么。

    // 3.1 Rust 属性

    // 在上一节中，我们使用 assert_eq! 宏来判断两个值是否相等，也有类似的 assert! 和 assert_ne! 等等宏实现类似的功能。
    // 它们的本质都是在特定情况下调用 panic! 宏。你可以取消下面这行代码的注释，然后尝试重新运行本实验，查看结果
    //panic!("exit now");
    //
    // 如果已做过上一节的实验，你会发现这条语句会调用到 modules/axruntime/src/lang_items.rs 中的函数。
    ...
    // 3. #[cfg_attr(feature = "axstd", no_mangle)] 中 cfg_attr 表示前一个参数成立时，生成后一个参数。这里表示当前 main 函数带 #[no_mangle] 标签，这在上一节的 trap.rs 中有介绍。
    // 4. #[cfg_attr(all(target_os = "none", not(test)), global_allocator)] 中的 all 表示逻辑与，not 表示逻辑非。类似的还有 any。

    // [rust] no_std 属性
    // 在目前每个实验的开头，都有一行 no_std 属性，表示当前环境中没有 Rust 标准库。
    ...
    // 2. 不需要依赖 syscall 的函数（如 axstd 中调用的 alloc 和上一节实验的 core）则可以直接调用；
    ...
    //
    // 与上一节不同，在一个CPU上没有架构天生用于支持多执行流。（并行指令和超线程等概念和本节内容无关，此处的CPU指从硬件上层的内核角度看到的逻辑核心）
    // 无论是通用寄存器，还是执行上一节魔法的各自特权寄存器，都只有一组。
    ...
    // 这个机制叫做“时钟中断”。简单来说，通过硬件设置，可以用上一节实验介绍的异常中断机制强行打断CPU，
    ...
    // 如果想要在多个执行流间安全地传递/访问数据，可以参考旧课程的 u_5_0, u_6_0, u_6_1 几个实验的代码，学习内核中的锁机制。
